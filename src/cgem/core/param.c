...
#define CGEM_PARAM_TYPE "param"
#define CGEM_PARAM_VALUE_KEY "value"

cgem_param_t *cgem_param_new(const cgem_char_t *name,
                             cgem_attribute_value_t *value,
                             cgem_node_t *owner)
{
    cgem_node_t *node = cgem_object_new(name, CGEM_PARAM_TYPE, owner);
    ...
    attribute = cgem_attribute_new(CGEM_PARAM_VALUE_KEY, value);
    if (!attribute) {
        cgem_attribute_value_free(value);
        cgem_node_free(node);
        return NULL;
...
const cgem_attribute_value_t *cgem_param_get_value(const cgem_param_t *param)
{
    ...
    attributes = cgem_node_get_attributes((cgem_node_t *) param);
    attribute = cgem_attributes_find(attributes, CGEM_PARAM_VALUE_KEY);
    if (!attribute) {
        return NULL;
    }
                                 ...
                                 cgem_attribute_value_t *value)
{
    ...
    if (!cgem_attributes_add(cgem_node_get_attributes((cgem_node_t *) param),
                             attribute)) {
        cgem_attribute_free(attribute);
        return false;
    }
